...
with st.sidebar:
    ...
    st.session_state.auto_refresh = st.checkbox("Auto-refresh", value=st.session_state.auto_refresh, help="Automatically refresh the workspace data at a fixed interval")
...
def render_workspace() -> None:
    ...
    if active_tab == "Needs attention":
        ...
        if attention_rows:
            ...
            df_attention = pd.DataFrame(attention_rows)[["reason", "status", "company", "title", "score", "job_id", "url", "timestamp"]]
            st.dataframe(
                df_attention,
                hide_index=True,
                column_config={
            ...
            if item["source"] == "history":
                ...
                if item["status"] in {"failed", "error"} and st.button(
                    "Retry application", key=f"retry-attention-{item['source_index']}",
                    icon=":material/refresh:", type="primary",
                    help="Reopen this application in the browser for another attempt",
                ):
                    existing = next((p for p in packages if p.get("job_id") == item["job_id"]), None)
                    if existing is None:
                        packages.append({
                            "job_id": item["job_id"],
                            "job": event.get("job", event),
